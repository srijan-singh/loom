 ...
 *
 * <ol>
 *   <li>Send the request to the LLM via {@link LLMGateway#send}.
 *   <li>Forward every TOKEN → {@link EventType#AGENT_TOKEN} SSE event.
 *   <li>On TOOL_CALL: invoke {@link MCPClient}, broadcast {@link EventType#AGENT_TOOL_CALL} +
 *       {@link EventType#AGENT_TOOL_RESULT}, then loop back with the result appended to history
 *       (one-turn).
 *   <li>On DONE: broadcast {@link EventType#SESSION_COMPLETED}.
 *   <li>On ERROR: broadcast {@link EventType#SESSION_FAILED}.
 * </ol>
 *
 * <p>Execution is dispatched to a background thread so the HTTP handler returns immediately while
 * the SSE stream continues asynchronously.
 */
@Slf4j
public class AgentRunner {
    ...
    /**
     * Submits the LLM call to a background thread and returns immediately. If a run is already
     * active for {@code sessionId} the new submission is rejected and the method returns {@code
     * false}; otherwise it returns {@code true} and the run is dispatched.
     *
    ...
    public boolean runAsync(String sessionId, LLMRequest request) {
        if (!activeSessions.add(sessionId)) {
            log.warn(
                    "Session {} already has an active run — rejecting duplicate submission",
                    sessionId);
            return false;
        }
        executor.submit(
    ...
    void runBlocking(String sessionId, LLMRequest request) {
        ...
        llmGateway.send(
                current[0],
                response -> handleFirstTurn(sessionId, current, hadToolCall, hadError, response));
        ...
        if (hadToolCall[0]) {
            ...
            llmGateway.send(current[0], response -> handleFinalTurn(sessionId, response));
        } else {
    ...
    private void handleFirstTurn(
        ...
        switch (response.getType()) {
            case LLMResponse.TOKEN:
                broadcast(sessionId, EventType.AGENT_TOKEN, Map.of("token", response.getContent()));
                break;
    ...
    private void handleFinalTurn(String sessionId, LLMResponse response) {
        switch (response.getType()) {
            ...
            case LLMResponse.ERROR:
                log.warn(
                        "LLM error (follow-up turn) for session={}: {}",
                        sessionId,
                        response.getContent());
                broadcast(
                        sessionId,
    ...
    /**
     * Builds a follow-up request that appends the tool result as an assistant + user exchange so
