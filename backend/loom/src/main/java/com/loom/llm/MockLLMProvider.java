 ...
 * Streams a short canned reply token-by-token so the SSE pipeline can be
...
public class MockLLMProvider implements LLMGateway {
    ...
    public void send(LLMRequest request, Consumer<LLMResponse> tokenConsumer) {
        log.warn("MockLLMProvider active — set ANTHROPIC_API_KEY or OPENAI_API_KEY for a real provider");
