...
/**
 * Provides the three queries every repository shares: {@link #findById}, {@link #findAll}, and
 * {@link #delete}.
 *
 * <p>Also exposes a shared {@link ObjectMapper} and JSON helpers ({@link #toJsonList} / {@link
 * #fromJsonList} / {@link #toJsonMap} / {@link #fromJsonMap}) for repositories that serialize list
 * or map columns.
 *
 * <p>Subclasses call {@code super(db, table)} then immediately call {@code setMapper(this::map)} in
 * their constructor, once their own {@code map()} method is in scope.
 *
...
abstract class BaseRepository<T> {
    ...
    /**
     * Serializes a {@code List<String>} to a JSON array string. Returns {@code "[]"} for null or
     * empty input.
     */
    ...
    /**
     * Deserializes a JSON array string to a {@code List<String>}. Returns an empty list for null or
     * blank input.
     */
    ...
    /**
     * Serializes a {@code Map<String, Object>} to a JSON object string. Returns {@code "{}"} for
     * null input.
     */
    protected static String toJsonMap(Map<String, Object> map) {
        ...
        try {
            ...
            throw new RuntimeException("Failed to serialize JSON map", e);
        }
    }

    /**
     * Deserializes a JSON object string to a {@code Map<String, Object>}. Returns an empty map for
     * null or blank input.
     */
     ...
     *
     * <p>The first column is always {@code id} and is excluded from the {@code ON CONFLICT … DO
     * UPDATE SET} clause (it never changes on conflict). All remaining columns are included in the
     * update.
     *
     * <p>Example — {@code upsert("skills", "id","name","tags","created_at","updated_at")} produces:
     *
     * <pre>{@code
    ...
    protected static String upsert(String table, String... cols) {
        ...
        if (!"id".equals(cols[0])) {
            throw new IllegalArgumentException(
                    "upsert: first column must be \"id\", got: " + cols[0]);
        }
     ...
     *
     * <p>SQLite stores nullable integers as {@code NULL}; {@link ResultSet#getLong} returns {@code
     * 0} for {@code NULL} and sets the {@code wasNull} flag. This helper checks that flag and
     * returns {@code null} instead of {@code 0}.
     *
